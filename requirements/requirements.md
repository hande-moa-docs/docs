...
| FR-03 | 최초 로그인 시 약관 동의       | SHOULD | '동의' 혹은 '로그아웃' 버튼 제공     |
| FR-04 | 마이페이지                | MUST   | 프로필, 신뢰도 점수, 참가 모임 목록 조회 |
...
* **마이페이지**: 프로필 수정(사진, 닉네임, 자기소개 등)과 내가 만든/참여한 모임 목록, 신뢰도 점수 및 등급 조회 지원

#### 4.2 모임 관리(호스트)

| ID    | 기능 상세    | 우선순위   | 비고                                                     |
|-------|----------|--------|--------------------------------------------------------|
| FR-08 | 모임 개설    | MUST   | 제목, 소개, 상품, 카테고리, 시간, 주소, 인원, 이미지 등                    |
| FR-09 | 모임 수정    | MUST   | PUT 방식으로 전체 정보 업데이트                                    |
| FR-10 | 신청자 관리   | MUST   | 신청 목록 확인                                               |
| FR-11 | 신청자 관리   | MUST   | PATCH 방식으로 승인(APPROVE) 또는 거절(REJECT)                   |
| FR-12 | 참가자 제거   | SHOULD | PUT 방식, 참가자를 논리적으로 삭제(내보내기)                            |
| FR-13 | 모임 상태 변경 | MUST   | 모임 시간에 따라 자동 변경되거나 호스트가 직접 변경 (모집중, 모집마감, 진행중, 종료, 취소) |
...
| ID    | 기능 상세       | 우선순위 | 비고                            |
|-------|-------------|------|-------------------------------|
| FR-14 | 모임 목록 조회    | MUST | 카드형 레이아웃 및 무한 스크롤             |
| FR-15 | 모임 필터/검색    | MUST | 지도 선택, 위치, 상품, 상태, 카테고리       |
| FR-16 | AI 챗봇 검색    | MUST | 자연어 질의를 통한 모임 검색 기능           |
| FR-17 | 모임 상세 조회    | MUST | 일반 사용자는 목록만, 로그인 회원만 상세 내용 확인 |
| FR-18 | 모임 참여 신청/취소 | MUST | 모집 마감 전 신청 가능, 중복 신청 방지       |
| FR-19 | 신청 상태 확인    | MUST | PENDING, APPROVED, REJECTED   |
...
| ID    | 기능 상세     | 우선순위   | 비고                                  |
|-------|-----------|--------|-------------------------------------|
| FR-20 | 댓글 읽기     | MUST   | 비참여자는 읽기 전용, 호스트/승인된 참여자는 쓰기, 수정 가능 |
| FR-21 | 댓글 작성     | MUST   |                                     |
| FR-22 | 댓글 수정, 삭제 | SHOULD |                                     |
...
| ID    | 기능 상세                      | 우선순위 | 비고                                       |
|-------|----------------------------|------|------------------------------------------|
| FR-23 | 피드백 작성                     | MUST | 모임 상태 `COMPLETED`, 본인 제외                 |
| FR-24 | 사용자 신뢰도 점수 시스템             | MUST | 초기 점수 300점                               |
| FR-25 | 신뢰도 등급 확인                  | MUST | '500점 이상: 좋음, 101~499점: 기본, 100점 이하: 경고' |
